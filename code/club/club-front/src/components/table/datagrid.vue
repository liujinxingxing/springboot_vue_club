<!--
封装datagrid  -带分页组件,滚动加载的表格 * author:阳春白雪
-->
<template>
  <div class="datagrid">
    <!-- 表格头部 -->
    ...
    >
      ...
      >
        ...
        <el-table-column
          ...
          label=" "
          width="45"
          v-if="config.expand && config.columns.length > 0"
        >
      ...
      >
        ...
        <div class="push-right" v-if="config.pagination.showPagination">
          <div class="push-right" style="margin-right: 8px" v-if="config.pagination.autoRollSelect">
            <el-select
...
export default {
  ...
  computed: {
    info(){
      var data = this.$store.state.render.common.datagrid;
      return data;
    },
  ...
  methods: {
    ...
    //设置datagrid对象
    setConfig: function() {
      this.config.sumText = this.info.sumText;
    ...
    toggleSelection: function(row, value = true) {
      if (row) {
        this.$refs.datagrid.toggleRowSelection(row,value);
      } else {
