...
<template>
  <div class="select-table u--select-common">
    <el-popover
      ...
      popper-class="u-Popver el-select-dropdown popperClass"
      :visible-arrow="false"
      :width="popoverWidth"
      trigger="click"
      @show="showPopper"
    ...
    >
      ...
      <el-input
        ...
        :placeholder="tempValues.length === 0 ? info.emptySelect : ''"
...
export default {
  ...
  computed: {
    info(){
      return this.$store.state.render.common.selectTable;
    },
    showClearIcon() {
  ...
  methods: {
     ...
     */
    setConfig: function() {
      this.config.emptyText = this.info.emptyText;
    ...
    nodeClick: function(row) {
      ...
      tempNode.name = row[this.config.name];
      if (this.config.multiple) {
...
.u--select-common{
  ...
  .input-padding-right .el-input__inner {
    padding-right: 45px;
  ...
  .u--medium .el-input__icon{
    height: 36px;
  }
  .u--small .el-input__icon{
    height: 26px;
  }
  .u--mini .el-input__icon{
    height: 22px;
  }
}
</style>
