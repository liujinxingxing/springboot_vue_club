      ...
      >
        <template scope="scope">
          <div class="table-btn-group">
            <u-button
              shape="text"
              @click="topRow(scope.row,0)"
              v-if="$store.state.permitteds[secretkey + 'top'] && scope.row.mode != 0"
            >{{info.untop}}</u-button>
            <u-button
              shape="text"
              @click="topRow(scope.row,1)"
              v-if="$store.state.permitteds[secretkey + 'top'] && scope.row.mode == 0"
            >{{info.top}}</u-button>
            <u-button
              shape="text"
...
export default {
  ...
  data() {
    return {
      keyword: "",
      secretkey: "btn:club:discuss:"
    };
  },
  computed: {
    info() {
      return this.$store.state.render.discuss.info;
    },
    datagrid() {
      var data = this.$store.state.render.discuss.datagrid;
      data.url = inter.discuss.list;
      return data;
    }
  },
  methods: {
    ...
    topRow(row,top) {
      this.$fetch(
        inter.discuss.top,
        Qs.stringify({ ids: [row.uuid] ,mode:top}, { arrayFormat: "repeat" })
      )
        .then(res => {
          if (res && res.success) {
            this.reload();
